...
func main() {
	...
	if *weather < 0 || *weather > 4 {
		elogFatalf("error: weather (-w) must be between 0 and 4")
	}

	args := pflag.Args()
	if len(args) != 2 {
		elogFatalf("error: missing arguments: usage: <executable> <player_showdown> <opponent_showdown> <flags>")
	}
	...
	if err != nil {
		elogFatalf("error: failed validating input '%s': %s", args[0], err)
	}
	opponentParty, err := cfg.validateInput(args[1])
	if err != nil {
		elogFatalf("error: failed validating input '%s': %s", args[1], err)
	}
